...
def generate_crime_hotspots(graph, n_crimes=1000):
    ...
    for x_pct, y_pct, count, spread in hotspot_specs:
        ...
        for _ in range(count):
            ...
            crime_type = np.random.choice(
                ["Robbery", "Theft", "Assault"], p=[0.5, 0.3, 0.2]
            )

            crimes_data.append(
                {"point_id": crime_id, "x": x, "y": y, "type": crime_type}
            )
            crime_id += 1
    ...
    for _ in range(remaining):
        ...
        crimes_data.append({"point_id": crime_id, "x": x, "y": y, "type": "Other"})
        crime_id += 1
...
def main():
    ...
    print("[2/5] Building spatial index with CGAL Delaunay triangulation...")
    start_time = time.time()
    ...
    print("      Distribution: 4 major hotspots + random background crimes")
    ...
    print("[4/5] Assigning crimes to nearest street junctions...")
    start_time = time.time()
    ...
    print(
        f"  • Crime assignment:   {assign_time:.2f}s ({N_CRIMES / assign_time:.0f} ops/s)"
    )
