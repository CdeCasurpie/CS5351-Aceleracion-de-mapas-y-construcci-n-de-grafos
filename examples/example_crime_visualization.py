...
def create_sample_crimes(num_crimes=200):
    ...
    for center_x, center_y, count, spread in hotspots:
        for _ in range(count):
            ...
            crimes_data.append(
                {"point_id": crime_id, "x": x, "y": y, "type": "Robbery"}
            )
            crime_id += 1
    ...
    for _ in range(num_crimes - crime_id):
        ...
        crimes_data.append({"point_id": crime_id, "x": x, "y": y, "type": "Theft"})
        crime_id += 1
...
def print_assignment_summary(assignments):
    ...
    print("Crime Assignment Summary:")
...
def main():
    ...
    for idx, row in assignments.head(5).iterrows():
        print(
            f"    Crime {int(row['point_id'])} at ({row['x']:.1f}, {row['y']:.1f}) "
    ...
    geojac.render_comparison(
        ...
        title="Crime Heatmap Visualization - Synthetic Network",
