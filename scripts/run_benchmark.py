...
from geojac.core.network import UrbanNetwork  # noqa: E402
from geojac.evaluation.metrics import (  # noqa: E402
    # descriptive
    NodeCountMetric,
    EdgeCountMetric,
...
from geojac.evaluation.reporting import ThesisReportGenerator  # noqa: E402
...
def _evaluate(raw: UrbanNetwork, net: UrbanNetwork, is_baseline: bool = False) -> dict:
...
def _run_geojac_master(network: UrbanNetwork) -> UrbanNetwork:
    ...
    segs_arr = np.ascontiguousarray(
        network.edges_df[["segment_id", "node_start", "node_end"]].values,
        dtype=np.float64,
    )
    ...
    result = acj_core.simplify_graph_acj_master_cgal(
        ...
        segs_arr,
        float(ACJ_ANGLE),
        float(ACJ_FACTOR_ADJ),
        float(ACJ_FACTOR_EPS),
        ACJ_WITH_INDEX,
    )
    ...
    if len(out_e) > 0:
        net.edges_df = pd.DataFrame(
            out_e, columns=["segment_id", "node_start", "node_end"]
        )
    return net
...
def run_benchmark(city_name: str = CITY) -> None:
    ...
    for name, net in competitors.items():
        ...
        result["reachability_preservation_%"] = sin_b[name][
            "reachability_preservation_%"
        ]
        con_b[name] = result
