...
class MapIndex:
    ...
    def _build_segment_index(self) -> None:
        ...
        segment_coords = self.graph_data.segments[
            ["x1", "y1", "x2", "y2"]
    ...
    def assign_to_endpoints(self, points_df: pd.DataFrame) -> pd.DataFrame:
        ...
        indices, distances = self._acj_core.match_point(
            point_coords, self._endpoint_index
        )
    ...
    def assign_to_segments(self, segments_df: pd.DataFrame) -> pd.DataFrame:
        ...
        indices, distances = self._acj_core.match_segment(
            point_coords, self._segment_index
        )
    ...
    def get_render_data(
        ...
        if assignments is not None:
            ...
            original_crime_counts = (
                assignments["assigned_node_id"].value_counts().to_dict()
            )
            ...
            for node_id in self.graph_data.nodes["node_id"]:
                ...
                neighbor_influence_sum = sum(
                    original_crime_counts.get(n_id, 0)
                    for n_id in self._adjacency_list[node_id]
                )
                smoothed_count = original_count + (
                    neighbor_influence * neighbor_influence_sum
                )
            ...
            node_id_to_idx = {
                node_id: i for i, node_id in enumerate(self.graph_data.nodes["node_id"])
            }
