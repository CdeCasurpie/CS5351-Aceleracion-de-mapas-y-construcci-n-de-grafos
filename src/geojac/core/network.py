...
class UrbanNetwork:
    ...
    @classmethod
    def from_dataframe(
        cls, nodes_df: pd.DataFrame, edges_df: pd.DataFrame
    ) -> "UrbanNetwork":
        network = cls()
        ...
        # Semantic
        nodes_meta_cols = [
            col for col in nodes_df.columns if col not in ["node_id", "x", "y"]
        ]
        if nodes_meta_cols:
            network.node_metadata = nodes_df.set_index("node_id")[
                nodes_meta_cols
            ].to_dict("index")
        ...
        edges_meta_cols = [
            ...
            if col not in ["segment_id", "node_start", "node_end"]
        ]
        if edges_meta_cols:
            network.edge_metadata = edges_df.set_index("segment_id")[
                edges_meta_cols
            ].to_dict("index")
    ...
    def from_networkx(cls, G) -> "UrbanNetwork":
        ...
        for idx, (osm_id, data) in enumerate(G.nodes(data=True)):
            ...
            nodes_data.append(
                {"node_id": idx, "x": data.get("x", 0.0), "y": data.get("y", 0.0)}
            )
