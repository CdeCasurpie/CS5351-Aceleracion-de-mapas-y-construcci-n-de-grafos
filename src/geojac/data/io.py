...
def load_map(
    city_name: str, cache_dir: str = "./cache", network_type: str = "drive"
) -> GraphData:
    """
    ...
    cache_filename = (
        f"{city_name.replace(' ', '_').replace(',', '')}_{network_type}.pkl"
    )
    ...
    for idx, (osm_id, data) in enumerate(G_proj.nodes(data=True)):
        ...
        nodes_data.append({"node_id": idx, "x": data["x"], "y": data["y"]})
    ...
    print(
        f"Converted to ACJ format: {len(nodes_df)} nodes, {len(segments_df)} segments"
    )
