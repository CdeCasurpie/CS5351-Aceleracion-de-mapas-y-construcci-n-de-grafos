...
class ACJVisualizer:
    ...
    def __init__(
        ...
        title: str = "ACJ Real-Time Analysis Tool",
        ...
        self.render_data = map_index.get_render_data(
            assignments, neighbor_influence=0.5, outlier_percentile=98.0
        )
    ...
    def _create_visuals(self):
        ...
        self.grid = visuals.GridLines(
            parent=self.view.scene, color=(0.5, 0.5, 0.5, 0.3)
        )
...
class ACJComparator:
    ...
    def __init__(
        self,
        map_index_left: MapIndex,
        map_index_right: MapIndex,
        ...
        self.render_data_left = map_index_left.get_render_data(
            assignments_left, outlier_percentile=98.0
        )
        self.render_data_right = map_index_right.get_render_data(
            assignments_right, outlier_percentile=98.0
        )
        ...
        self.view_left = self.grid.add_view(
            row=0, col=0, border_color="gray", camera=self.camera
        )
        self.view_right = self.grid.add_view(
            row=0, col=1, border_color="gray", camera=self.camera
        )
        ...
        self.debugger_text_left = visuals.Text(
            "",
            pos=(15, 15),
            anchor_x="left",
            anchor_y="bottom",
        ...
        self.debugger_text_right = visuals.Text(
            "",
            pos=(canvas_size[0] - 15, 15),
            anchor_x="right",
            anchor_y="bottom",
            color="white",
            font_size=10,
        ...
        self.label_text_left = visuals.Text(
            "LEFT",
            pos=(center_left, top_pos + 25),
            anchor_x="center",
            anchor_y="top",
            color="#00ff00",
            font_size=16,
        ...
        self.title_text_left = visuals.Text(
            title_left,
            pos=(center_left, top_pos),
            anchor_x="center",
            anchor_y="top",
            color="white",
            font_size=16,
        ...
        self.label_text_right = visuals.Text(
            "RIGHT",
            pos=(center_right, top_pos + 25),
            anchor_x="center",
            anchor_y="top",
            color="#ff8800",
            font_size=16,
        ...
        self.title_text_right = visuals.Text(
            title_right,
            pos=(center_right, top_pos),
            anchor_x="center",
            anchor_y="top",
            color="white",
            font_size=16,
    ...
    def _on_key_press(self, event):
        ...
        if event.key in ("n", "N"):
            self.visibility_state["nodes"] = not self.visibility_state["nodes"]
            self.nodes_left.visible = self.nodes_right.visible = self.visibility_state[
                "nodes"
            ]
        elif event.key in ("l", "L"):
            self.visibility_state["segments"] = not self.visibility_state["segments"]
            self.segments_left.visible = self.segments_right.visible = (
                self.visibility_state["segments"]
            )
...
def render_heatmap(
    map_index: MapIndex, assignments: pd.DataFrame, title: str = "Crime Density Heatmap"
):
